...
const checks = [
  ...
  ["seller/evidence/model/prompt invalidate cache", () => { const base = lib.semanticFingerprint({ projectId: "p", companyId: "c", sellerContextFingerprint: seller.fingerprint, evidence }); for (const changed of [{ sellerContextFingerprint: "other" }, { evidence: [...evidence, { ...evidence[0], id: "ev_1111111111111111" }] }, { model: "other" }, { promptVersion: "other" }]) assert.notEqual(base, lib.semanticFingerprint({ projectId: "p", companyId: "c", sellerContextFingerprint: seller.fingerprint, evidence, ...changed })); }],
   ["v2 fingerprint bypasses v1 cache", () => assert.notEqual(lib.semanticFingerprint({ projectId: "p", companyId: "c", sellerContextFingerprint: seller.fingerprint, evidence, promptVersion: "fix08-company-understanding-v1" }), lib.semanticFingerprint({ projectId: "p", companyId: "c", sellerContextFingerprint: seller.fingerprint, evidence }))],
   ["v4 generic seller-relative role definitions", () => { assert.notEqual(lib.semanticFingerprint({ projectId: "p", companyId: "c", sellerContextFingerprint: seller.fingerprint, evidence, promptVersion: "fix08-company-understanding-v3" }), lib.semanticFingerprint({ projectId: "p", companyId: "c", sellerContextFingerprint: seller.fingerprint, evidence })); for (const role of ["POTENTIAL_BUYER means", "SELLER_COMPETITOR means", "ADJACENT_VENDOR means", "PARTNER_POSSIBLE means", "UNKNOWN means"]) assert.match(lib.COMPANY_SEMANTIC_SYSTEM_PROMPT, new RegExp(role)); assert.doesNotMatch(lib.COMPANY_SEMANTIC_SYSTEM_PROMPT, /\\b(?:SOC|MSSP|MDR)\\b/i); }],
