...
| Use Case | People + Company search |
...
const results = await exa.search("engineering leaders at AI infrastructure companies", {
  "type": "auto",
  "numResults": 10,
  "category": "people"
});
...
```json
{
  "query": "engineering leaders at AI infrastructure companies",
  "type": "auto",
  "numResults": 10,
  "category": "people"
}
...
## Use Exa Agent for list building and enrichment

Exa Agent is well suited when the product needs to find people or companies, verify multiple facts, enrich each record, and return a structured list with source URLs.

**When building with Exa Agent:**
- Use Agent for multi-step people/company workflows: list building, sourcing, account research, and enrichment.
- Ask for evidence-backed fields in `outputSchema` so every returned person or company includes the fields your product needs.
- Pass existing accounts or leads as `input.data` when enriching CRM rows, and preserve stable IDs such as `crm_id` in the output.
...
```json
{
  "query": "Find 25 AI infrastructure companies that raised funding recently and are hiring designers. Include company, category, funding signal, hiring signal, and source URL.",
  "effort": "auto",
  "outputSchema": {
    "type": "object",
    "properties": {
...
Use `category: "people"` to search for people by role, expertise, or what they work on.
...
**Tips:**
- For people: use `category: "people"` - find by role, expertise, or what they work on
- For companies: use `category: "company"` - find by industry, criteria, or attributes
...
results = exa.search(
    "engineering leaders at AI infrastructure companies",
    category="people",
    num_results=10
)
