...
router.get("/projects/:projectId/research", requireAuth, asyncRoute(async (req, res) => {
  ...
  const payload = await Promise.all(companies.map(async ({ projectCompany, company }) => {
    ...
    return {
      ...
      buyerRole: projectCompany.buyerRole,
      intelligenceStage: projectCompany.buyerRole === "UNKNOWN" ? "NEEDS_MINIMUM_INTELLIGENCE" : "ROLE_RESOLVED",
      nextAction: projectCompany.buyerRole === "POTENTIAL_BUYER" ? "Research now" : "Review company qualification",
...
router.post("/projects/:projectId/companies/:projectCompanyId/research", requireAuth, asyncRoute(async (req, res) => {
  ...
  if ("stopped" in result) {
    res.json(ExecuteCompanyResearchResponse.parse({
      ...
      buyerRole: result.buyerRole ?? "UNKNOWN",
      intelligenceStage: result.intelligenceStage ?? "NOT_STARTED",
      stopCode: result.stopCode ?? "STOPPED",
      progress: result.progress ?? "buyer_research_gated",
      nextAction: result.nextAction ?? "Review the company role.",
      question: null,
      job: null,
      evidenceCount: 0,
      factProposalCount: 0,
  ...
  res.json(ExecuteCompanyResearchResponse.parse({
    ...
    intelligenceStage: "BUYER_RESEARCH",
    stopCode: null,
    progress: "buyer_research_complete",
    nextAction: "Review the resulting evidence.",
