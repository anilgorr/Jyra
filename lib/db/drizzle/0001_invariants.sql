-- JYRA database invariants: triggers, functions, partial indexes and
-- constraints the application relies on for immutability, provenance and
-- scope isolation. This file is executed by the production migrator
-- (`pnpm --dir lib/db migrate`) and by the development helper
-- (`node lib/db/scripts/apply-invariants.mjs`). Every statement is
-- idempotent: functions use CREATE OR REPLACE, triggers are dropped before
-- they are created, indexes use IF NOT EXISTS and constraints are guarded by
-- a pg_constraint lookup, so the file can be re-run safely.
...
-- Prediction snapshots are append-only. A direct DELETE is always rejected.
-- A cascade (pg_trigger_depth() > 1, e.g. deleting the owning campaign or
...
BEGIN
  IF TG_OP = 'DELETE' AND pg_trigger_depth() > 1 THEN
    SELECT c.frozen_at INTO campaign_frozen_at
      FROM market_readiness_campaigns c WHERE c.id = OLD.campaign_id;
    IF campaign_frozen_at IS NOT NULL
       AND current_setting('jyra.allow_frozen_teardown', true) IS DISTINCT FROM 'on' THEN
      RAISE EXCEPTION 'market readiness campaign is frozen; its prediction snapshots cannot be deleted'
        USING ERRCODE = '55000';
    END IF;
    RETURN OLD;
  END IF;
...
-- A frozen campaign is immutable in every direction, including DELETE. The
-- campaign row itself rejects DELETE once frozen, so a cascading project or
-- organization teardown fails loudly instead of destroying the frozen cohort,
-- adjudications and prediction snapshots. Every child row also rejects DELETE
-- while its campaign is frozen, whether the DELETE is direct or a cascade.
-- The only bypass is the explicit session setting
...
BEGIN
  IF OLD.frozen_at IS NOT NULL
     AND current_setting('jyra.allow_frozen_teardown', true) IS DISTINCT FROM 'on' THEN
    RAISE EXCEPTION 'market readiness campaign is frozen and cannot be deleted'
      USING ERRCODE = '55000';
  END IF;
  RETURN OLD;
END; $$;
DROP TRIGGER IF EXISTS market_readiness_campaigns_freeze_deletes ON market_readiness_campaigns;
CREATE TRIGGER market_readiness_campaigns_freeze_deletes
...
BEGIN
  ...
  SELECT c.frozen_at INTO campaign_frozen_at
    FROM market_readiness_campaigns c WHERE c.id = OLD.campaign_id;
  -- NOT FOUND means the (unfrozen) campaign row was already removed by the
  -- same cascade; a frozen campaign would have rejected its own DELETE first.
  IF FOUND AND campaign_frozen_at IS NOT NULL
     AND current_setting('jyra.allow_frozen_teardown', true) IS DISTINCT FROM 'on' THEN
    RAISE EXCEPTION 'market readiness campaign is frozen; % rows cannot be deleted', TG_TABLE_NAME
      USING ERRCODE = '55000';
  END IF;
  RETURN OLD;
END; $$;
DO $$ DECLARE t text; BEGIN
  FOREACH t IN ARRAY ARRAY[
    'market_readiness_cohort_items','market_readiness_blind_gold_reviews','market_readiness_adjudications',
    'market_readiness_salesperson_reviews','market_readiness_manual_outcomes','market_readiness_prediction_snapshots',
    'market_readiness_processing_attempts','market_readiness_experiments','market_readiness_experiment_assignments',
    'market_readiness_outcome_import_batches','market_readiness_rollout_decisions','market_readiness_audits'
  ] LOOP
    EXECUTE format('DROP TRIGGER IF EXISTS %I ON %I', t || '_freeze_deletes', t);
    EXECUTE format('CREATE TRIGGER %I BEFORE DELETE ON %I FOR EACH ROW EXECUTE FUNCTION reject_market_readiness_frozen_child_delete()', t || '_freeze_deletes', t);
  END LOOP;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'market_readiness_campaign_target_exactly_200'
      AND conrelid = 'market_readiness_campaigns'::regclass
  ) THEN
    ALTER TABLE market_readiness_campaigns
      ADD CONSTRAINT market_readiness_campaign_target_exactly_200 CHECK (target_count = 200);
  END IF;
END $$;
