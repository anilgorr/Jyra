...
export const signalDefinitionsTable = pgTable(
  "signal_definitions",
  {
    ...
    factRequirements: jsonb("fact_requirements").$type<Record<string, unknown>>().notNull().default({}),
    ...
    fitImpact: real("fit_impact").notNull().default(0),
    sourcePreferences: jsonb("source_preferences").$type<string[]>().notNull().default([]),
    status: text("status").notNull().default("APPROVED"),
...
export const signalsTable = pgTable(
  "signals",
  {
    ...
    categorySnapshot: text("category_snapshot").notNull().default("CUSTOM"),
    contextSnapshot: jsonb("context_snapshot").$type<Record<string, unknown>>().notNull().default({}),
    generationMethod: text("generation_method").notNull().default("DETERMINISTIC"),
    generatorVersion: text("generator_version").notNull().default("1.0"),
    observedAt: timestamp("observed_at", { withTimezone: true }).notNull().defaultNow(),
    needImpactSnapshot: real("need_impact_snapshot"),
    timingImpactSnapshot: real("timing_impact_snapshot"),
    fitImpactSnapshot: real("fit_impact_snapshot"),
