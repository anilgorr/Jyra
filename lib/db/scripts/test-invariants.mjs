...
try {
  ...
  const frozenDeleteTriggers = await client.query(`
    SELECT tgname FROM pg_trigger
    WHERE tgname LIKE 'market\\_readiness\\_%\\_freeze\\_deletes'
      AND tgenabled <> 'D' AND NOT tgisinternal
  `);
  assert.equal(frozenDeleteTriggers.rowCount, 13, "frozen campaign and every child table must reject DELETE");

  const exactly200 = await client.query(`
    SELECT 1 FROM pg_constraint
    WHERE conname = 'market_readiness_campaign_target_exactly_200'
      AND conrelid = 'market_readiness_campaigns'::regclass
  `);
  assert.equal(exactly200.rowCount, 1, "campaign target must be constrained to exactly 200");
