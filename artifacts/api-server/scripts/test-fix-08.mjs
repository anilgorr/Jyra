...
const fingerprintInput = { projectId: "p", companyId: "c", sellerContextFingerprint: seller.fingerprint, canonicalName: profile.canonicalName, canonicalDomain: "acme.example", evidence };
...
const checks = [
  ...
  ["strict schemas and enums", () => { assert.equal(lib.companySemanticOutputSchema.safeParse({ ...valid, business_model: "INVENTED" }).success, false); assert.equal(lib.companySemanticOutputSchema.safeParse({ ...valid, canonical_industry: "ARBITRARY_INDUSTRY" }).success, false); }],
   ["OpenAI strict response schema", () => { assert.equal(lib.companySemanticResponseSchema.strict, true); assert.deepEqual(lib.companySemanticResponseSchema.schema.required, ["primary_business", "business_model", "canonical_industry", "products_services", "commercial_role", "confidence", "reason", "evidence_ids", "missing_information"]); assert.equal(lib.companySemanticResponseSchema.schema.additionalProperties, false); }],
  ...
  ["fingerprint stable", () => assert.equal(lib.semanticFingerprint(fingerprintInput), lib.semanticFingerprint({ ...fingerprintInput, evidence: [...evidence].reverse() }))],
  ["seller/evidence/model/prompt invalidate cache", () => { const base = lib.semanticFingerprint(fingerprintInput); for (const changed of [{ sellerContextFingerprint: "other" }, { evidence: [...evidence, { ...evidence[0], id: "ev_1111111111111111" }] }, { model: "other" }, { promptVersion: "other" }]) assert.notEqual(base, lib.semanticFingerprint({ ...fingerprintInput, ...changed })); }],
   ["v2 fingerprint bypasses v1 cache", () => assert.notEqual(lib.semanticFingerprint({ ...fingerprintInput, promptVersion: "fix08-company-understanding-v1" }), lib.semanticFingerprint(fingerprintInput))],
   ["v4 generic seller-relative role definitions", () => { assert.notEqual(lib.semanticFingerprint({ ...fingerprintInput, promptVersion: "fix08-company-understanding-v3" }), lib.semanticFingerprint(fingerprintInput)); for (const role of ["POTENTIAL_BUYER means", "SELLER_COMPETITOR means", "ADJACENT_VENDOR means", "PARTNER_POSSIBLE means", "UNKNOWN means"]) assert.match(lib.COMPANY_SEMANTIC_SYSTEM_PROMPT, new RegExp(role)); assert.doesNotMatch(lib.COMPANY_SEMANTIC_SYSTEM_PROMPT, /\\b(?:SOC|MSSP|MDR)\\b/i); }],
  ["project relative cache keys", () => assert.notEqual(lib.semanticFingerprint({ ...fingerprintInput, projectId: "project-a" }), lib.semanticFingerprint({ ...fingerprintInput, projectId: "project-b" }))],
