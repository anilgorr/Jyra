...
router.patch("/projects/:projectId/market-readiness/campaigns/:campaignId", requireAuth, asyncRoute(async(req,res)=>{try{const p=api.UpdateMarketReadinessCampaignParams.parse(req.params),b=api.UpdateMarketReadinessCampaignBody.parse(req.body),a=await campaignAccess(p.projectId,p.campaignId,getAuthenticatedUserId(res));if(a.campaign.frozenAt)throw new Error("CAMPAIGN_FROZEN_IMMUTABLE");const [r]=await db.update(marketReadinessCampaignsTable).set(b).where(eq(marketReadinessCampaignsTable.id,p.campaignId)).returning();json(res,api.UpdateMarketReadinessCampaignResponse,r);}catch(e){fail(res,e);}}));
for (const action of ["start","pause","cancel"] as const) router.post(`/projects/:projectId/market-readiness/campaigns/:campaignId/${action}`,requireAuth,asyncRoute(async(req,res)=>{try{const p=campaignParams.parse(req.params),a=await campaignAccess(p.projectId,p.campaignId,getAuthenticatedUserId(res));if(a.campaign.frozenAt&&action!=="cancel")throw new Error("CAMPAIGN_FROZEN_IMMUTABLE");const state=action==="start"?"DISCOVERING":action==="pause"?"PARTIAL":"CANCELLED";const[r]=await db.update(marketReadinessCampaignsTable).set({state}).where(eq(marketReadinessCampaignsTable.id,p.campaignId)).returning();res.json(r);}catch(e){fail(res,e);}}));
router.post("/projects/:projectId/market-readiness/campaigns/:campaignId/resume",requireAuth,asyncRoute(async(req,res)=>{try{
  ...
  if(a.campaign.frozenAt)throw new Error("CAMPAIGN_FROZEN_IMMUTABLE");
  const row=await db.transaction(async tx=>{
    await tx.execute(sql`select id from market_readiness_campaigns where id=${p.campaignId} for update`);
    ...
    if(!campaign)throw new Error("CAMPAIGN_NOT_FOUND");
    const[fenced]=await tx.select().from(marketReadinessProcessingAttemptsTable).where(and(eq(marketReadinessProcessingAttemptsTable.campaignId,p.campaignId),eq(marketReadinessProcessingAttemptsTable.error,"LEASE_EXPIRED_RECONCILIATION_REQUIRED"),eq(marketReadinessProcessingAttemptsTable.state,"FAILED"))).orderBy(desc(marketReadinessProcessingAttemptsTable.createdAt)).limit(1);
    if(!fenced)throw new Error("NO_FENCED_ATTEMPT_TO_RETRY");
    const retryKey=`${fenced.idempotencyKey}:retry:${fenced.id}`;
    const[existing]=await tx.select().from(marketReadinessProcessingAttemptsTable).where(and(eq(marketReadinessProcessingAttemptsTable.campaignId,p.campaignId),eq(marketReadinessProcessingAttemptsTable.idempotencyKey,retryKey))).limit(1);
    ...
    if(retryCents<=0||campaign.spentCents+campaign.reservedCents+retryCents>campaign.paidCapCents)throw new Error("CAMPAIGN_HARD_CAP_EXCEEDED");
    await tx.insert(marketReadinessProcessingAttemptsTable).values({organizationId:campaign.organizationId,projectId:campaign.projectId,campaignId:campaign.id,cohortItemId:fenced.cohortItemId,kind:fenced.kind,idempotencyKey:retryKey,state:"PENDING",reservedCents:retryCents});
    const[updated]=await tx.update(marketReadinessCampaignsTable).set({reservedCents:sql`${marketReadinessCampaignsTable.reservedCents}+${retryCents}`,state:fenced.kind==="DISCOVERY"?"DISCOVERING":"RUNNING"}).where(eq(marketReadinessCampaignsTable.id,campaign.id)).returning();
    return updated!;
  });
  res.json(row);
}catch(e){fail(res,e);}}));
router.post("/projects/:projectId/market-readiness/campaigns/:campaignId/freeze",requireAuth,asyncRoute(async(req,res)=>{try{
  ...
  const r=await db.transaction(async tx=>{
    ...
    const[cohort,leases,adjudications,predictions]=await Promise.all([
    ...
    if(leases.length)throw new Error("FREEZE_REQUIRES_NO_ACTIVE_LEASES");
