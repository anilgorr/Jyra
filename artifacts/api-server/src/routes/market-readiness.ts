...
router.post("/projects/:projectId/market-readiness/campaigns/:campaignId/freeze",requireAuth,asyncRoute(async(req,res)=>{try{
  ...
  const r=await db.transaction(async tx=>{
    ...
    const[cohort,activeAttempts,reviews,adjudications,predictions,attempts]=await Promise.all([
      ...
      tx.select().from(marketReadinessBlindGoldReviewsTable).where(and(eq(marketReadinessBlindGoldReviewsTable.campaignId,p.campaignId),eq(marketReadinessBlindGoldReviewsTable.organizationId,a.project.organizationId),eq(marketReadinessBlindGoldReviewsTable.projectId,p.projectId))),
...
for(const action of ["start","complete"] as const)router.post(`/projects/:projectId/market-readiness/campaigns/:campaignId/experiments/:experimentId/${action}`,requireAuth,asyncRoute(async(req,res)=>{try{const p=api.GetMarketReadinessExperimentParams.parse(req.params),a=await campaignAccess(p.projectId,p.campaignId,getAuthenticatedUserId(res));const[e]=await db.select().from(marketReadinessExperimentsTable).where(and(eq(marketReadinessExperimentsTable.id,p.experimentId),eq(marketReadinessExperimentsTable.campaignId,p.campaignId))).limit(1);if(!e)throw new Error("EXPERIMENT_NOT_FOUND");const assignments=await db.select().from(marketReadinessExperimentAssignmentsTable).where(eq(marketReadinessExperimentAssignmentsTable.experimentId,e.id));if(action==="start"&&(!a.campaign.frozenAt||e.state!=="ASSIGNED"||e.startedAt||assignments.length!==200||assignments.filter(x=>x.arm==="TREATMENT").length!==100||assignments.filter(x=>x.arm==="CONTROL").length!==100))throw new Error("EXPERIMENT_START_PRECONDITIONS_FAILED");if(action==="complete"&&(e.state!=="RUNNING"||!e.startedAt||e.completedAt||!assignments.length))throw new Error("EXPERIMENT_COMPLETE_PRECONDITIONS_FAILED");const now=new Date();const[r]=await db.update(marketReadinessExperimentsTable).set(action==="start"?{state:"RUNNING",startedAt:now}:{state:"COMPLETED",completedAt:now}).where(and(eq(marketReadinessExperimentsTable.id,p.experimentId),eq(marketReadinessExperimentsTable.state,e.state),action==="start"?isNull(marketReadinessExperimentsTable.startedAt):isNull(marketReadinessExperimentsTable.completedAt))).returning();if(!r)throw new Error("EXPERIMENT_STATE_CHANGED");json(res,api.ActionMarketReadinessExperimentResponse,r);}catch(e){fail(res,e);}}));
router.post("/projects/:projectId/market-readiness/campaigns/:campaignId/blind-reviews",requireAuth,asyncRoute(async(req,res)=>{try{
  const p=api.CreateMarketReadinessBlindReviewParams.parse(req.params),b=api.CreateMarketReadinessBlindReviewBody.parse(req.body),u=getAuthenticatedUserId(res),a=await campaignAccess(p.projectId,p.campaignId,u);
  const r=await db.transaction(async tx=>{
    await tx.execute(sql`select id from market_readiness_campaigns where id=${p.campaignId} and organization_id=${a.project.organizationId} and project_id=${p.projectId} for update`);
    const[campaign]=await tx.select().from(marketReadinessCampaignsTable).where(and(eq(marketReadinessCampaignsTable.id,p.campaignId),eq(marketReadinessCampaignsTable.organizationId,a.project.organizationId),eq(marketReadinessCampaignsTable.projectId,p.projectId))).limit(1);
    if(!campaign||campaign.state!=="REVIEWING"||campaign.frozenAt)throw new Error("REVIEWS_REQUIRE_UNFROZEN_REVIEWING_STATE");
    const[item]=await tx.select().from(marketReadinessCohortItemsTable).where(and(eq(marketReadinessCohortItemsTable.id,b.cohortItemId),eq(marketReadinessCohortItemsTable.campaignId,p.campaignId),eq(marketReadinessCohortItemsTable.organizationId,a.project.organizationId),eq(marketReadinessCohortItemsTable.projectId,p.projectId))).limit(1);
    if(!item)throw new Error("COHORT_ITEM_SCOPE_MISMATCH");
    const[reviews,adjudications]=await Promise.all([
      tx.select().from(marketReadinessBlindGoldReviewsTable).where(and(eq(marketReadinessBlindGoldReviewsTable.campaignId,p.campaignId),eq(marketReadinessBlindGoldReviewsTable.cohortItemId,item.id))),
      tx.select().from(marketReadinessAdjudicationsTable).where(and(eq(marketReadinessAdjudicationsTable.campaignId,p.campaignId),eq(marketReadinessAdjudicationsTable.cohortItemId,item.id))),
    ]);
    if(adjudications.length)throw new Error("ADJUDICATION_ALREADY_RECORDED");
    if(reviews.some(x=>x.reviewerId===u))throw new Error("BLIND_REVIEW_ALREADY_RECORDED");
    if(reviews.length>=2)throw new Error("BLIND_REVIEW_LIMIT_REACHED");
    const[row]=await tx.insert(marketReadinessBlindGoldReviewsTable).values({...b,organizationId:a.project.organizationId,projectId:p.projectId,campaignId:p.campaignId,reviewerId:u}).returning();
    return row;
  });
  json(res,api.CreateMarketReadinessBlindReviewResponse,r,201);
}catch(e){fail(res,e);}}));
router.post("/projects/:projectId/market-readiness/campaigns/:campaignId/adjudications",requireAuth,asyncRoute(async(req,res)=>{try{
  ...
  json(res,api.CreateMarketReadinessAdjudicationResponse,r,201);
}catch(e){fail(res,e);}}));
...
router.post("/projects/:projectId/market-readiness/campaigns/:campaignId/outcomes",requireAuth,asyncRoute(async(req,res)=>{try{const p=api.CreateMarketReadinessOutcomeParams.parse(req.params),b=api.CreateMarketReadinessOutcomeBody.parse(req.body),u=getAuthenticatedUserId(res),a=await campaignAccess(p.projectId,p.campaignId,u);const item=await scopedCohortItem(a.campaign,b.cohortItemId);const assignments=await db.select().from(marketReadinessExperimentAssignmentsTable).where(and(eq(marketReadinessExperimentAssignmentsTable.campaignId,p.campaignId),eq(marketReadinessExperimentAssignmentsTable.organizationId,a.project.organizationId),eq(marketReadinessExperimentAssignmentsTable.projectId,p.projectId),eq(marketReadinessExperimentAssignmentsTable.cohortItemId,item.id)));const assignment=uniqueCampaignAssignment(assignments,item.normalizedDomain);const[experiment]=await db.select().from(marketReadinessExperimentsTable).where(and(eq(marketReadinessExperimentsTable.id,assignment.experimentId),eq(marketReadinessExperimentsTable.campaignId,p.campaignId))).limit(1);if(!experiment||!["RUNNING","COMPLETED"].includes(experiment.state))throw new Error("OUTCOMES_REQUIRE_STARTED_EXPERIMENT");const occurredAt=validateOutcomeOccurredAt(experiment.startedAt,b.occurredAt);const[r]=await db.insert(marketReadinessManualOutcomesTable).values({...b,experimentAssignmentId:assignment.id,occurredAt,organizationId:a.project.organizationId,projectId:p.projectId,campaignId:p.campaignId,recordedBy:u}).onConflictDoNothing().returning();if(!r){const[x]=await db.select().from(marketReadinessManualOutcomesTable).where(and(eq(marketReadinessManualOutcomesTable.campaignId,p.campaignId),eq(marketReadinessManualOutcomesTable.idempotencyKey,b.idempotencyKey))).limit(1);if(!x)throw new Error("OUTCOME_ALREADY_RECORDED_FOR_ASSIGNMENT");json(res,api.CreateMarketReadinessOutcomeResponse,x);return;}json(res,api.CreateMarketReadinessOutcomeResponse,r,201);}catch(e){fail(res,e);}}));
...
router.put("/projects/:projectId/market-readiness/campaigns/:campaignId/rollout",requireAuth,asyncRoute(async(req,res)=>{try{
  ...
  const[cohort,reviews,adjudications,predictions,experiments,outcomes,attempts]=await Promise.all([
    db.select().from(marketReadinessCohortItemsTable).where(eq(marketReadinessCohortItemsTable.campaignId,p.campaignId)),
    db.select().from(marketReadinessBlindGoldReviewsTable).where(eq(marketReadinessBlindGoldReviewsTable.campaignId,p.campaignId)),
  ...
  catch(error){evaluationErrors.push(error instanceof Error?error.message:"INVALID_BLIND_REVIEW_COVERAGE");}
  ...
  const experimentReady=!!completedExperiment?.startedAt&&!!completedExperiment.completedAt&&assignments.length===200&&new Set(assignments.map(x=>x.cohortItemId)).size===200&&treatment.length===100&&control.length===100;
  const arm=(rows:typeof assignments)=>{const ids=new Set(rows.map(x=>x.id));const attributed=outcomes.filter(x=>x.experimentAssignmentId&&ids.has(x.experimentAssignmentId));return{total:ids.size,observed:new Set(attributed.map(x=>x.experimentAssignmentId)).size,meetingOrOpportunity:attributed.filter(x=>x.outcome==="MEETING"||x.outcome==="OPPORTUNITY").length,badFit:attributed.filter(x=>x.outcome==="BAD_FIT").length};};
  const assignedIds=new Set(assignments.map(x=>x.id));
  const invalidOutcome=outcomes.find(x=>!assignedIds.has(x.experimentAssignmentId)||!completedExperiment?.startedAt||x.occurredAt<completedExperiment.startedAt);
  const commercial=invalidOutcome
    ?{pass:false,lift:null,badFitIncrease:null,reason:"INVALID_OR_PRE_EXPERIMENT_OUTCOME"}
    :commercialGate(arm(treatment),arm(control));
...
router.post("/projects/:projectId/market-readiness/campaigns/:campaignId/outcomes/import",requireAuth,asyncRoute(async(req,res)=>{try{
  ...
  const[experiment]=await db.select().from(marketReadinessExperimentsTable).where(eq(marketReadinessExperimentsTable.campaignId,p.campaignId)).limit(1);
  if(!experiment||!["RUNNING","COMPLETED"].includes(experiment.state))throw new Error("OUTCOMES_REQUIRE_STARTED_EXPERIMENT");
  for(const row of rows)validateOutcomeOccurredAt(experiment.startedAt,row.occurredAt);
  const result=await db.transaction(async tx=>{
    ...
    for(const {row,item,assignment} of resolved){const[inserted]=await tx.insert(marketReadinessManualOutcomesTable).values({organizationId:a.project.organizationId,projectId:p.projectId,campaignId:p.campaignId,cohortItemId:item.id,experimentAssignmentId:assignment.id,importBatchId:batch.id,outcome:row.outcome,occurredAt:new Date(row.occurredAt),recordedBy:u,idempotencyKey:`${b.idempotencyKey}:${row.domain}`}).onConflictDoNothing().returning({id:marketReadinessManualOutcomesTable.id});if(!inserted)throw new Error(`OUTCOME_ALREADY_RECORDED_FOR_ASSIGNMENT:${row.domain}`);}
