...
// The migrate script may read a local .env first, but it must run the real
// migrator and nothing else — never drizzle-kit push, which diffs live schema.
assert.match(pkg.scripts.migrate, /^node (--env-file-if-exists=\S+ )?\.\/scripts\/migrate\.mjs$/);
